...
export function SettingsModal({
  ...
  return (
    <div className="fixed inset-0 z-50">
      ...
      >
        {/* 头部 */}
        <div className="flex items-center justify-between px-5 py-4 pt-safe" style={{ borderBottom: '1px solid var(--mi-border)' }}>
          <button
            onClick={onClose}
          ...
          <h2 className="text-lg font-semibold" style={{ color: 'var(--mi-text-primary)' }}>设置</h2>
          <div className="w-10" />
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-6 pb-safe">
          ...
          {/* ── 外观 ── */}
          <section>
            <h3 className="text-[13px] font-medium mb-3 px-1" style={{ color: 'var(--mi-text-tertiary)' }}>外观</h3>
            <div className="rounded-2xl overflow-hidden" style={{ background: 'var(--mi-card)', boxShadow: 'var(--mi-shadow)' }}>
              <button
                onClick={onToggleTheme}
                className="w-full flex items-center justify-between px-5 py-4 active:bg-gray-50 dark:active:bg-gray-700/50 transition-colors"
              >
                <div className="flex items-center gap-3.5">
                  ...
                  <span className="text-[15px]" style={{ color: 'var(--mi-text-primary)' }}>
                ...
                >
                  <div className="w-6 h-6 bg-white rounded-full shadow-sm mx-0.5" />
                </div>
              </button>
            </div>
          </section>
          ...
          {/* ── 分类管理 ── */}
          <section>
            <h3 className="text-[13px] font-medium mb-3 px-1" style={{ color: 'var(--mi-text-tertiary)' }}>分类管理</h3>
            <div className="rounded-2xl overflow-hidden" style={{ background: 'var(--mi-card)', boxShadow: 'var(--mi-shadow)' }}>
              {/* 新增分类 */}
              <div className="flex gap-3 p-4" style={{ borderBottom: categories.length > 0 ? '1px solid var(--mi-divider)' : 'none' }}>
                <input
                  type="text"
                  placeholder="新分类名称..."
                  value={newCatName}
              ...
              {categories.length === 0 ? (
                <div className="flex flex-col items-center py-10 text-center">
                  <Tag className="w-12 h-12 mb-4" style={{ color: 'var(--mi-border)' }} />
                  <p className="text-[15px] mb-1" style={{ color: 'var(--mi-text-tertiary)' }}>还没有分类</p>
                  <p className="text-[13px]" style={{ color: 'var(--mi-text-tertiary)', opacity: 0.6 }}>
                    创建分类来整理你的胶囊
                  </p>
                </div>
              ) : (
                      ...
                      {editingId === cat.id ? (
                        <>
                          <input
                            ...
                            className="flex-1 text-[15px] bg-transparent outline-none border-0"
                        ...
                        <>
                          <span className="flex-1 text-[15px]" style={{ color: 'var(--mi-text-primary)' }}>
                            {cat.name}
                          </span>
                          <button
          ...
          {/* ── 数据管理 ── */}
          <section>
            <h3 className="text-[13px] font-medium mb-3 px-1" style={{ color: 'var(--mi-text-tertiary)' }}>数据管理</h3>
            <div className="rounded-2xl overflow-hidden" style={{ background: 'var(--mi-card)', boxShadow: 'var(--mi-shadow)' }}>
              <button
                onClick={handleExport}
                className="w-full flex items-center gap-3.5 px-5 py-4 active:bg-gray-50 dark:active:bg-gray-700/50 transition-colors"
                style={{ borderBottom: '1px solid var(--mi-divider)' }}
              >
                <Download className="w-5 h-5 text-blue-500" />
                <span className="text-[15px]" style={{ color: 'var(--mi-text-primary)' }}>导出备份</span>
              </button>
              <button
              ...
              >
                ...
                <span className="text-[15px]" style={{ color: 'var(--mi-text-primary)' }}>导入数据</span>
              </button>
              <input ref={fileInputRef} type="file" accept=".json" onChange={handleImport} className="hidden" />
              {importError && (
                <p className="text-[13px] px-5 pb-4" style={{ color: '#FF4444' }}>{importError}</p>
              )}
            </div>
          </section>
          ...
          <section className="text-center pt-6 pb-8">
            <div className="flex items-center justify-center gap-2 mb-2">
              ...
              <span className="text-[13px] font-medium" style={{ color: 'var(--mi-text-tertiary)' }}>
                灵感胶囊 · 本地版
              </span>
            </div>
            <p className="text-[12px]" style={{ color: 'var(--mi-text-tertiary)', opacity: 0.5 }}>
              v1.0 · 数据仅存储在本地设备
            </p>
          </section>
        </div>
