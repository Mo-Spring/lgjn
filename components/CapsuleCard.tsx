...
export function CapsuleCard({
  ...
  if (viewMode === 'list') {
    return (
      ...
      >
        {/* 选择指示器 */}
        {isSelectionMode && (
          <div className="flex-shrink-0 mt-1 animate-mi-check-in">
            <div
        ...
        <div className="flex-1 min-w-0">
          ...
          {note.pinned && !isSelectionMode && (
            <div className="flex items-center gap-1.5 mb-2">
              ...
              <span className="text-xs font-medium" style={{ color: 'var(--mi-orange)' }}>置顶</span>
            </div>
          )}

          {/* 标题 */}
          {note.title && (
            <h3 className="font-semibold text-base mb-2 truncate" style={{ color: 'var(--mi-text-primary)' }}>
          ...
          {/* 内容预览 */}
          {note.content && (
            <p className="text-sm line-clamp-2 leading-relaxed" style={{ color: 'var(--mi-text-secondary)' }}>
          ...
          <div className="flex items-center gap-3 mt-3">
            ...
            <span className="text-xs" style={{ color: 'var(--mi-text-tertiary)' }}>
  ...
  return (
    ...
    >
      ...
      {/* 标题 */}
      {note.title && (
        <h3 className="font-semibold text-base mb-2 line-clamp-2" style={{ color: 'var(--mi-text-primary)' }}>
          {highlightText(note.title, searchQuery)}
        </h3>
      )}
      ...
      {/* 内容预览 */}
      {note.content && (
        <p className="text-sm line-clamp-4 leading-relaxed" style={{ color: 'var(--mi-text-secondary)' }}>
          {highlightText(note.content, searchQuery)}
        </p>
      )}

      {/* 底部：分类 + 时间 */}
      <div className="flex items-center justify-between mt-3 pt-3" style={{ borderTop: '1px solid var(--mi-divider)' }}>
        {category ? (
          <span
            className="text-xs px-3 py-1 rounded-full font-medium"
            style={{ background: 'var(--mi-hover)', color: 'var(--mi-text-tertiary)' }}
